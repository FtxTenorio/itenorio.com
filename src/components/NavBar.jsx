...
export const Navbar = ({ currentPage, setCurrentPage }) => {
  ...
  const getLinkClass = (pageName) => {
    ...
    return `px-4 py-2 rounded-full text-sm font-semibold transition-all duration-300 cursor-pointer bg-transparent border-none flex items-center gap-1 ${isActive
        ...
        <div className="absolute left-0 mt-2 w-48 bg-[#1A1F2B] border border-gray-700 rounded-lg shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 transform origin-top-left group-hover:translate-y-0 -translate-y-2 overflow-hidden">
          <div className="py-2">
            <button
              ...
              className={`w-full text-left px-4 py-2 text-sm flex items-center gap-2 transition-colors ${currentPage === 'games'
            ...
            <button
              ...
              className={`w-full text-left px-4 py-2 text-sm flex items-center gap-2 transition-colors ${currentPage === 'music'
                ? 'text-white bg-white/10 font-bold border-l-4 border-[#0078d7]'
                : 'text-gray-300 hover:text-white hover:bg-white/5 border-l-4 border-transparent'
                }`}
            >
        ...
        <div className="absolute right-0 mt-2 w-48 bg-[#1A1F2B] border border-gray-700 rounded-lg shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 transform origin-top-right group-hover:translate-y-0 -translate-y-2 overflow-hidden">
          <div className="py-2">
            <button
              ...
              className={`w-full text-left px-4 py-2 text-sm flex items-center justify-between transition-colors ${isMinecraft
  ...
  return (
    <NavBarRaw navButtons={NavButtons} navbarThemeClass={navbarThemeClass} currentPage={currentPage} isMinecraft={isMinecraft} setCurrentPage={handleNavClick}>
            ...
            <div className="w-full pt-1">
              ...
              {isMobileAboutOpen && (
                <div className="pl-6 pr-3 py-2 space-y-1 border-l-2 border-white/20 ml-3 mt-1 animate-fade-in">
                  <button
                    onClick={() => handleNavClick('games')}
                    className={`w-full text-left px-3 py-2 rounded-md text-sm font-medium transition-all flex items-center gap-2 ${currentPage === 'games'
                  ...
                  <button
                    onClick={() => handleNavClick('music')}
                    className={`w-full text-left px-3 py-2 rounded-md text-sm font-medium transition-all flex items-center gap-2 ${currentPage === 'music'
                      ? 'text-white bg-white/10 border-l-4 border-[#0078d7]'
                      : 'text-gray-400 hover:text-white hover:bg-white/5 border-l-4 border-transparent'
                      }`}
                  >
            ...
            <div className="w-full pt-1 border-t border-[rgba(255,255,255,0.05)] mt-1">
              ...
              {isMobileServersOpen && (
                <div className="pl-6 pr-3 py-2 space-y-1 border-l-2 border-[#3C8527]/50 ml-3 mt-1 animate-fade-in">
                  <button
                    onClick={() => handleNavClick('minecraft')}
                    className={`w-full text-left px-3 py-2 rounded-md text-sm font-medium transition-all flex items-center justify-between ${isMinecraft
